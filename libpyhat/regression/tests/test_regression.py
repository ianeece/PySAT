...
mock = mock.Mock(spec=regression)

def test_PLS():
    regress = mock(method=['PLS'], yrange=[0.0, 100.0],
                         params=[{'n_components': 0,'scale': False}])
    print(regress)
    #
    regress.assert_called_with(method=['PLS'], yrange=[0.0, 100.0],
                         params=[{'n_components': 0,'scale': False}])
    # mock.fit(0, 0)
    # mock.fit.assert_called_with(0, 0)
